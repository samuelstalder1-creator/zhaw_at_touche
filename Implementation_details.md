...
- The embedding-divergence baseline family in `setup100`, `setup101`, and `setup102` defaults to fitting its threshold during `touche-train` and reusing the saved state during `touche-embed-divergence`, with validation-based calibration as a fallback.
